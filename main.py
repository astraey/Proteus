...
def main():
    proteusCoin = BlockChain()
    proteusCoin.addBlock(Block(1, "10/10/2017", {"account": "Anna","amount": 25,"action": "buy"}))
    proteusCoin.addBlock(Block(2, "11/01/2017", {"account": "Joe","amount": 10,"action": "buy"}))
    proteusCoin.addBlock(Block(3, "12/01/2017", {"account": "Katie","amount": 20,"action": "buy"}))
    proteusCoin.addBlock(Block(4, "12/07/2017", {"account": "Ethan","amount": 4,"action": "buy"}))
    proteusCoin.printBlockChain()
    # no tampering in our block chain yet so should be true here
    print "Chain valid? " + str(proteusCoin.isChainValid())
    # now lets tamper the block chain and see what happens
    proteusCoin.chain[1].data = {"account": "Anna","amount": 100,"action": "buy"}
    print "Chain valid? " + str(proteusCoin.isChainValid())
